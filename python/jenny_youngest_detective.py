...
In order to find out what the word is, you should use the sticker (array of 3 numbers) to retrive 3 letters from the comment (string) that 
create the word.
...
def missing(nums, s):
    try:
        return "".join(s.replace(" ", '').lower()[x] for x in sorted(nums))
    except:
