...
(2) return an ordered string, 26 places long, corresponding to the number of occurences for each
...
def decrypt(test_key):
    ...
    count = Counter(test_key)
        
    alphabet = 'abcdefghijklmnopqrstuvwxyz'
    
    return ''.join(str(count[x]) for x in alphabet)