/*
https://leetcode.com/problems/game-play-analysis-iv/description/?envType=study-plan-v2&envId=top-sql-
...
player_next_login as 
(
  select
    player_id,
    date_add(first_login, interval 1 day) as next_login
  from player_id_first_login
),
...
(
  select
    ...
    event_date
  from activity
  where (player_id, event_date) in (select player_id, next_login from player_next_login)
)

select round(
  (select count(*) from consecutive_count) /
