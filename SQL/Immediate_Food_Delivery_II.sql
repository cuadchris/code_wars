...
with cus_first_order as (
    select distinct customer_id,
        ...
        min(customer_pref_delivery_date) over(partition by customer_id) as first_pref
    from delivery
),
first_order_match_pref as (
    select *
    from cus_first_order
    where first_order = first_pref
)
select round(
        (
            select count(1)
            from first_order_match_pref
        ) / (
            select count(1)
            from cus_first_order
        ) * 100,
